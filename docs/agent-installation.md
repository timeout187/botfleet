...
| `BOTFLEET_AGENT_CAPABILITIES`     | no             | Comma-separated, from `node`, `pm2`, `docker`, `discordjs`, `eris`, `ai-worker`, `custom-executable`. Defaults to `node,custom-executable` - `node` matches `workload-runner.ts`'s only implemented runtime, so an unconfigured agent is schedulable for `runner.type: "node"` workloads out of the box. |
