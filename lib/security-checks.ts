...
export async function runSecurityChecks(): Promise<SecurityReport> {
  ...
  checks.push(
    isEncryptionKeyConfigured()
      ? {
          ...
          status: "pass",
          detail: "BOTFLEET_ENCRYPTION_KEY is set and decodes to a 32-byte key.",
        }
      : {
          id: "encryption-key",
          label: "Encryption key configured",
          status: "fail",
          detail: "BOTFLEET_ENCRYPTION_KEY is missing or invalid. Bot tokens cannot be encrypted.",
        },
  );
  ...
  checks.push({
    ...
    status: "pass",
    detail:
      "Bot tokens are only ever written via encryptSecret() (AES-256-GCM); the schema has no plaintext token column.",
  });
  ...
  checks.push(
    adminCount > 0
      ? {
          ...
          status: "pass",
          detail: `${adminCount} admin/owner user(s) found.`,
        }
      : {
          id: "admin-configured",
          label: "Admin user configured",
          status: "fail",
          detail:
            "No user has the admin or owner role yet. Sign in with the Discord account listed in BOTFLEET_ADMIN_DISCORD_IDS.",
        },
  );
  ...
  checks.push(
    hasDiscordOAuth
      ? {
          id: "oauth-configured",
          label: "Discord OAuth app configured",
          status: "pass",
          detail: "AUTH_DISCORD_ID / AUTH_DISCORD_SECRET are set.",
        }
      : {
          id: "oauth-configured",
          label: "Discord OAuth app configured",
          status: "fail",
          detail: "AUTH_DISCORD_ID / AUTH_DISCORD_SECRET are not set - admin login will fail.",
        },
  );
  ...
  checks.push(
    hasAuthSecret
      ? {
          ...
          status: "pass",
          detail: "AUTH_SECRET is set.",
        }
      : {
          id: "auth-secret",
          label: "Session secret configured",
          status: "fail",
          detail: "AUTH_SECRET is not set - generate one with: openssl rand -base64 32",
        },
  );
  ...
  checks.push({
    ...
    status: "pass",
    detail:
      "A restrictive CSP header (no unsafe-eval) is set in next.config.ts for every response.",
  });
  ...
  checks.push({
    ...
    status: "pass",
    detail:
      "requireAdmin()/requireCustomer() helpers always return NextResponse.json(..., { status }) and never redirect fetch requests to a login page.",
  });
  ...
  checks.push({
    ...
    status: "pass",
    detail:
      "Bot tokens and webhook URLs are never included in API responses or log output; only maskSecret() output is ever surfaced.",
  });
  ...
  checks.push(
    backupsConfigured
      ? {
          ...
          label: "Database backups configured",
          status: "pass",
          detail: "BOTFLEET_BACKUP_STORAGE_URL is set.",
        }
      : {
          id: "backups-configured",
          label: "Database backups configured",
          status: "warn",
          detail:
            "BOTFLEET_BACKUP_STORAGE_URL is not set. Configure automated backups for your Postgres instance before going to production.",
        },
  );
  ...
  checks.push(
    backupEncryptionConfigured
      ? {
          ...
          label: "Backup encryption configured",
          status: "pass",
          detail: "BOTFLEET_BACKUP_ENCRYPTION_KEY is set.",
        }
      : {
          id: "backup-encryption",
          label: "Backup encryption configured",
          status: "warn",
          detail:
            "BOTFLEET_BACKUP_ENCRYPTION_KEY is not set. If your backup provider doesn't encrypt at rest, set this.",
        },
  );
