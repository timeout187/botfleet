...
export function TriggerDeploymentDialog() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setResult(
      body.restartsQueued > 0
        ? `Deployed. Queued staggered restarts for ${body.restartsQueued} online bot(s).`
        : "Deployed. No restarts queued (no online bots, or maintenance mode is on).",
    );
    setOpen(false);
        ...
        <DialogContent title="Trigger a deployment">
          <form onSubmit={onSubmit} className="space-y-4">
            <p className="text-xs text-zinc-500">
              Runs every registered plugin&apos;s deployment hooks (see /admin/plugins), then queues
              a staggered restart (15s apart) for every currently-online bot - skipped if
